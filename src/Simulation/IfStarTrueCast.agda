module Simulation.IfStarTrueCast where
...
sim-if⋆-true-cast : ∀ {Σ Σ′ gc gc′} {M M′ N′ μ μ′ PC PC′} {A A′ T′} {ℓ}
                        {c̅ : CExpr l ℓ ⇒ ⋆}
    → (vc  : LVal PC)
    → (vc′ : LVal PC′)
    → let ℓv  = ∥ PC  ∥ vc  in
       let ℓv′ = ∥ PC′ ∥ vc′ in
       [] ; [] ∣ Σ ; Σ′ ∣ gc ; gc′ ∣ ℓv ; ℓv′ ⊢
         M ⊑ if⋆ ($ true ⟨ cast (id Bool) c̅ ⟩) T′ M′ N′ ⇐ A ⊑ A′
    → Σ ⊑ₘ Σ′
    → Σ ; Σ′ ⊢ μ ⊑ μ′
    → PC ⊑ PC′ ⇐ gc ⊑ gc′
    → SizeEq μ μ′
    ...
    → ∃[ N ] (M ∣ μ ∣ PC —↠ N ∣ μ) ×
              (let ℓ′ = ∥ c̅ ∥ₗ 𝓋′ in
               [] ; [] ∣ Σ ; Σ′ ∣ gc ; gc′ ∣ ℓv ; ℓv′ ⊢
                     N ⊑ prot (stamp!ₑ PC′ vc′ ℓ′) (stamp!ₑ-LVal vc′) ℓ′ M′ (T′ of ⋆)
                  ⇐ A ⊑ A′)
        ...
        ⟨ ir-base (inj 𝓋) x , ⊑-base c̅⊑c̅′ ⟩ →
          ⟨ _ ,
            ...
            (let ∣c̅∣≼∣c̅′∣ = security-prec _ _ (inj 𝓋) 𝓋′ c̅⊑c̅′ in
             ⊑-prot! M⊑M′ (stamp!ₑ-prec vc vc′ PC⊑PC′ ∣c̅∣≼∣c̅′∣)
                   (≡→≼ (stamp!ₑ-security vc)) (≡→≼ (stamp!ₑ-security vc′)) ∣c̅∣≼∣c̅′∣) ⟩
sim-if⋆-true-cast vc vc′ (⊑-castl {c = c} M⊑M′ c⊑A′) Σ⊑Σ′ μ⊑μ′ PC⊑PC′ size-eq 𝓋′ =
  case sim-if⋆-true-cast vc vc′ M⊑M′ Σ⊑Σ′ μ⊑μ′ PC⊑PC′ size-eq 𝓋′ of λ where
  ⟨ N , M↠N , N⊑N′ ⟩ →
