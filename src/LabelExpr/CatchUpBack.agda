...
... | ⟨ l ℓ ⟪ c̅′₁ ⟫ , M′↠V′ , v-v (v-cast i) (⊑-castr {g⊑g₁′ = l⊑l} {l⊑l} ⊑-l ℓ⊑c̅′₁) ⟩ =
  ...
  ⟨ c̅″ , c̅′↠c̅″ , v-v (inj 𝓋′) c̅′⊑c̅″ ⟩ →
    ⟨ l _ ⟪ c̅″ ⟫ , ↠ₑ-trans (plug-congₑ M′↠V′) (_ —→⟨ comp i ⟩ _ —→⟨ cast c̅′↠c̅″ (inj 𝓋′) ⟩ _ ∎) ,
      v-v {g⊑g′ = g₂⊑g₂′} (v-cast ⟨ inj 𝓋′ , (λ ()) ⟩) (⊑-cast {g₁⊑g₁′ = l⊑l} {g₂⊑g₂′} ⊑-l c̅′⊑c̅″) ⟩
catchup-back (v-cast i) (⊑-castl {g₁⊑g′ = l⊑l} {g₂⊑g′} V⊑M′ c̅⊑g′)
  with catchup-back {g⊑g′ = l⊑l} v-l V⊑M′
... | ⟨ blame p , M′↠⊥ , v-⊥ prec ⟩ =
  ⟨ blame p , M′↠⊥ , v-⊥ {g⊑g′ = g₂⊑g′} (⊑-castl {g₁⊑g′ = l⊑l} {g₂⊑g′} (⊑-blame {g⊑g′ = l⊑l} ⊢l) c̅⊑g′) ⟩
... | ⟨ V′ , M′↠V′ , v-v {g⊑g′ = g⊑g′} v′ M⊑V′ ⟩
  with prec→⊢ {g⊑g′ = g⊑g′} M⊑V′ | g⊑g′
... | ⟨ ⊢l , _ ⟩ | l⊑l =
    ⟨ V′ , M′↠V′ , v-v {g⊑g′ = g₂⊑g′} v′ (⊑-castl {g₁⊑g′ = l⊑l} {g₂⊑g′} M⊑V′ c̅⊑g′) ⟩
catchup-back v (⊑-castr V⊑M′ x) = {!!}
catchup-back v (⊑-blame {g⊑g′ = g⊑g′} ⊢V) =
  ⟨ _ , _ ∎ , v-⊥ {g⊑g′ = g⊑g′} (⊑-blame {g⊑g′ = g⊑g′} ⊢V) ⟩
