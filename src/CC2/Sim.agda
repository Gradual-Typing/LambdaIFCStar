...
... | ⟨ V , V-raw V-ƛ , L↠V , ⊑-lam g⊑g′ A⊑A′ N⊑N′ ⟩ =
  ...
  ⟨ W , w , M↠W , W⊑M′ ⟩ →
    ...
    ⟨ Σ , Σ′ , _ , μ , ♣ ,
      ⊑-prot (substitution-pres-⊑ ⊑*-∅ Σ⊑Σ′ N⊑N′ (value-⊑-pc W⊑M′ w vM′)) (stampₑ-prec vc vc′ PC⊑PC′) (≡→≼ (stampₑ-security vc)) (≡→≼ (stampₑ-security vc′)) eq eq′ , μ⊑μ′ ⟩
...
... | ⟨ addr _ ⟨ cast (ref c d) c̅ ⟩ , V-cast V-addr (ir-ref 𝓋) ,
  ...
  ⟨ Σ , Σ′ , _ , μ ,
    ...
    μ⊑μ′ ⟩
sim {Σ} {Σ′} {gc} {gc′} {μ₁ = μ} {PC = PC} {PC′} vc vc′
    (⊑-deref!l M⊑M′ eq eq′) Σ⊑Σ′ μ⊑μ′ PC⊑PC′ (deref {v = v′} μ′a≡V′)
  with catchup {μ = μ} {PC} (V-raw V-addr) M⊑M′
...
... | V-cast V-addr (ir-ref 𝓋) | ⊑-cast (⊑-addr a b) (⊑-ref c⊑c′ d⊑d′ c̅⊑c̅′) =
  ...
  ⟨ Σ , Σ′ , _ , cons-μ _ W₁ w₁ _ , ♣ , ⊑-const , ⊑μ-update μ⊑μ′ (value-⊑-pc W₁⊑W′ w₁ w′) w₁ w′ a b ⟩
